...
- **Import-your-library affordance**: surface the manual library export/import channel (bookmarklet → JSON → file picker) as an onboarding option that also works for private profiles — see [`../archive/manual-library-export-feasibility.md`](../archive/manual-library-export-feasibility.md)
...
- Related: `docs/acts/act3-ready-for-everyone.md` has the offline/bookmarklet export-format research item, now studied in [`../archive/manual-library-export-feasibility.md`](../archive/manual-library-export-feasibility.md) — the anonymous store fixture format should eventually align with whatever static export shape we land on.

...
- ~~`InputStateTracker.getProgressiveSpeed()` (acceleration ramp, `:59-72`) exists but has **zero
  callers** anywhere in the codebase.~~ **Resolved 2026-07-23** (task 6) — left in place
  intentionally rather than wired or deleted, tracked as `progressive-speed-movement-unwired` in
  `docs/tech-debt.md`.
...
- Gamepad controller navigation works for basic movement and game selection — **true as of 2026-07-24** (task 11): movement/look were already real and covered by tests (task 8); game selection, menu-open, and fullscreen-toggle were found dead for every device (not just gamepad) and fixed in task 11, closing [`gamepad-button-actions-unconsumed`](../tech-debt.md#id-gamepad-button-actions-unconsumed).
- Input method detection and seamless switching between keyboard and gamepad
- ~~Camera roll toggle, movement acceleration, configurable speeds — bindings/state exist, application doesn't; wire or remove~~ **Resolved**: roll wired (task 3), movement acceleration deliberately left unwired as tracked debt (task 6)
...
- **Toggle UI visibility ("clean screenshot" mode)** — nice-to-have, not scheduled. `InputAction.ToggleUI` existed briefly (bound to `Tab`/gamepad View button) but was removed 2026-07-24 (task 11) since no consumer was ever designed for it — hiding UI chrome (docked settings/lighting buttons) is a real, separable idea worth picking up later, but inventing a feature to fill an unused binding would have been scope creep on task 11. Revisit if screenshot/photo-mode use cases make this worth building for real.
- **In-app fullscreen toggle** — rejected as an *input action*, not as a feature. `InputAction.ToggleFullscreen` existed briefly (bound to keyboard `F`) but was removed 2026-07-24 (task 11): **F11 already triggers the browser's own native fullscreen** in every major browser, independent of any in-page JavaScript, so a dedicated keybind would just duplicate a shortcut keyboard/mouse users already have. The concrete gap this note anticipated — gamepad-only users have no keyboard to press F11 with — turned out to already have a fix: `GraphicsSettingsPanel` already has a `fullscreen-enabled` checkbox wired to the real Fullscreen API (found 2026-07-24, task 12), so no new UI was needed either. What's still missing is a way for a gamepad-only user to actually *reach* that checkbox — the pause menu has no gamepad-driven focus navigation at all, tracked as [`gamepad-menu-navigation-unimplemented`](../tech-debt.md#id-gamepad-menu-navigation-unimplemented).
...
11. ✅ **Route `Interact`/`OpenMenu` through the input stack** — closes [`gamepad-button-actions-unconsumed`](../tech-debt.md#id-gamepad-button-actions-unconsumed), done 2026-07-24 per `docs/plans/input-action-routing-plan.md` (implemented, then revised twice the same day after design reviews — see the plan's "Revision history"). Keyboard fires directly from the real `keydown` DOM event via a new `InputStateTracker.onRawKeyDown` callback — no polling, no per-frame diffing, since keyboard already has an unambiguous press edge. Mouse deliberately has no equivalent path: a real mouse click already has its own independent dispatch in `SystemUICoordinator`, entirely separate from bindings, so nothing routes it through here. Gamepad has no native press event, so `DeviceDetector.pollGamepads()` (already polling every frame for connect/disconnect) tracks per-button state and emits `InputEventTypes.GamepadButtonPressed` on a transition. Both keyboard and gamepad resolve through new `InputActionResolver.handleRawKeyPress()`/`handleGamepadButtonPress()` methods (using a new `BindingResolver.findButtonActionsBoundTo()` helper), which emit a *specific* event per action — `InputEventTypes.OpenMenuPressed` / `InputEventTypes.InteractPressed` — rather than one generic event with an `actionId` field for consumers to check; `InputActionResolver`'s job is deciding which action fired, so it does that fully. No dispatcher class: `PauseMenuManager` listens for `OpenMenuPressed` directly and calls its own `toggle()` (replacing the old hardcoded `Escape`-only `keydown` listener); `SystemUICoordinator` listens for `InteractPressed` and simulates a click at the reticle position (the existing `SceneCanvasClick` event with center-screen NDC), and owns the gamepad/VR reticle (shown while a non-pointer device is connected and the menu is closed). Both `ToggleUI` and `ToggleFullscreen` were removed entirely rather than built — see the Stretch section for why. `InputActionResolver` also caches its connected-profile-id set (refreshed only on `InputEventTypes.DevicesChanged`, not rebuilt every frame) — `updateFrame()` itself is back to pure continuous-state resolution with no edge-detection bookkeeping in it. Found and fixed two real latent bugs along the way: (1) pausing didn't actually stop gamepad-driven camera movement, since `WebXRCoordinator`'s render-loop callback was never gated on pause state — `InputManager` now has `pause()`/`resume()` that gate camera movement/rotation application only, while `updateFrame()` (gamepad polling) keeps running so `OpenMenu`'s close-press can still be detected while paused; (2) `DeviceDetector.pollGamepads()` only flagged a device-list change on gamepad *disconnect*, never *connect* via polling. Verified via unit tests across `InputStateTracker`, `BindingResolver`, `InputActionResolver`, `InputManager`, `DeviceDetector`, `PauseMenuManager`, and `SystemUICoordinator`; full suite green.
12. ✅ **Per-frame gamepad-poll audit + gamepad reticle setting** — done 2026-07-24, a direct follow-up to task 11. Audited every per-frame call in the `InputManager.updateFrame()` → `InputActionResolver.updateFrame()` → `DeviceDetector.pollGamepads()` path for redundant work; found one real one: `InputActionResolver.updateFrame()` called `navigator.getGamepads()` itself, a second, separate call to the same browser API `DeviceDetector.pollGamepads()` had *just* called one line above for its own connect/disconnect and button-press-edge tracking. Fixed by having `pollGamepads()` return the connected-gamepads array it already computed, so `updateFrame()` reuses it instead of re-querying and re-filtering `navigator.getGamepads()` every frame regardless of gamepad state. Everything else on the path (the `BindingResolver.resolve()` loop over the fixed `INPUT_ACTION_DEFINITIONS` list, `CameraInputApplier`'s handful of map lookups) is a small fixed-size pass, not worth trimming further. Also confirmed (no fix needed) that the gamepad/VR reticle (`SystemUICoordinator`) was already event-driven, not polled — visibility only changes on `DevicesChanged`/`MenuOpen`/`MenuClose`. Added `AppSettings.inputGamepadReticleEnabled` (default `true`) with a checkbox in `ControlsPanel`'s Input Devices section, alongside the existing Mouse Look toggle; `SystemUICoordinator.updateReticleVisibility()` now also gates on this setting and reacts live via `AppSettingsEventTypes.Changed` (event-driven, not polled). Also confirmed (no fix needed) that `GamepadStandard`'s `OpenMenu` binding (button 9 / Menu-Start) already exists by default. Surfaced but deliberately not built: gamepad-only users can't yet navigate *inside* the pause menu at all (see the Stretch section's fullscreen note and [`gamepad-menu-navigation-unimplemented`](../tech-debt.md#id-gamepad-menu-navigation-unimplemented)) — scoped as its own feature rather than folded into this follow-up.

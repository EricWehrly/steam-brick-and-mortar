...
- **In-app fullscreen toggle** — rejected, not just deferred. `InputAction.ToggleFullscreen` existed briefly (bound to keyboard `F`) but was removed 2026-07-24 (task 11): **F11 already triggers the browser's own native fullscreen** in every major browser, independent of any in-page JavaScript, so an in-app toggle would just duplicate a shortcut users already have. No plan to revisit unless a concrete reason emerges that F11 doesn't cover (e.g. a packaged/kiosk build without browser chrome).

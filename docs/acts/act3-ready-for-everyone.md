...
- **Baked artwork/appdetails freshness (TTL or manual purge)** — once F2P artwork/appdetails are baked into a release (see [F2P Artwork Bake](../plans/f2p-artwork-bake-plan.md)), the client never re-checks Steam's CDN for that appid — a game that later leaves Early Access with new box art stays stuck on whatever was baked at release time, with no expiry. Before this was pre-baked, staleness wasn't a concern: the app always fetched from Steam directly and the existing "clear cache" flow could force a refetch. Two options, not mutually exclusive: (a) a TTL on baked entries — past some age, prefer the CDN over the baked/cached copy for that appid; (b) a dedicated "purge image cache" action (alongside the existing appdetails cache-clear) that lets every game's artwork re-fetch fresh from Steam's CDN on demand. Needs [Image/Texture Pipeline](../architecture/image-texture-pipeline.md) and `GameArtworkProvider`'s baked-artwork-manifest awareness revisited alongside this.

...
- **Loading placeholder boxes** — idea captured, not designed: when a game box's slot is known but its artwork hasn't resolved yet (especially relevant now that artwork resolution can lag noticeably behind library load — see the offline-first plan above), drop in a cheap, generic "art incoming" placeholder (e.g. a simple spinner/shimmer) rather than leaving the slot blank, swapped for real artwork once it resolves. Deliberately **not** a full label-box render for every pending game — too resource-heavy to generate at that scale for what's meant to be a brief, generic placeholder. Natural tie-in to [Game Box Construction Chain](../features/game-box-construction-chain.md)'s staged pipeline (a placeholder could occupy the box between `ArtworkRenderRequested` and `ArtworkTextureResolved`) if/when that lands, but doesn't require it. Also the resolution for [Liminal Mode](../features/liminal-mode.md)'s observed "games arrive slightly after shelves" gap (2026-07-30): shelves are pure procedural geometry with no network dependency, while game artwork must be fetched/decoded/uploaded, so some visible lag between the two is structural, not a bug — a placeholder box closes the gap without forcing shelves to wait on network fetches. Discuss and scope when picked up.

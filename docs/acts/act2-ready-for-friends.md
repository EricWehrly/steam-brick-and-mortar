...
- **CDN artwork traffic** — researched (2026-07-09), two plans drafted, neither built yet. [Texture Cache Refactor Plan](../plans/texture-cache-refactor-plan.md) (Plan 1): fixes a pre-existing double-fetch bug and adds cross-session MID-tier caching, so returning users stop re-downloading artwork every session — bigger, cheaper win than anything new, do this first. [F2P Artwork Bake](../plans/f2p-artwork-bake-plan.md) (Plan 2): bakes the small (~1MB) anonymous-store artwork set into the release, same spirit as the appdetails bake above; connected-user libraries are deliberately *not* blanket-baked (per-user, potentially hundreds of MB — Steam's CDN, not our infra, is the right thing serving that traffic, and it's built for public fan-out at a much larger scale than we'd generate).

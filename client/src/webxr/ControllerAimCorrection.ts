 ...
 * (VRControllerPointer) - imports this instead of each carrying its own correction, so the two can
 * never silently disagree about which way the controller is aiming (direct request, 2026-09-02:
 * "the blue ray line ... and the game box that actually opens don't line up").
 *
 ...
 * comment) - confirmed live (2026-08-19) against the VR settings menu specifically: an uncorrected
 * beam read as aiming up and away rather than forward. Pitching the direction down compensates.
 *
 * That -15 degree value was tuned against one scenario (a menu floating ~0.9m in front of the
 * camera) - it has NOT been re-verified for the game box's own scenarios (held close in-hand in

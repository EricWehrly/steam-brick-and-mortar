...
export class LegacyStorePropsRenderer implements IStorePropsRenderer {
    ...
    public async setupProps(config: PropsConfig = {}): Promise<void> {
        ...
        try {
            ...
            console.log('✅ Legacy props setup completed')
        } catch (error) {
            console.error('❌ Failed to set up legacy props:', error)
    ...
    private async createShelfRow(rowIndex: number, shelfCount: number, games: SteamGameData[] = []): Promise<void> {
        ...
        for (let i = 0; i < shelfCount; i++) {
            ...
            const shelfGroup = await this.createLegacyShelfWithGames(shelfPosition, rowIndex, i, shelfGames)
    ...
    /**
     * Create a single shelf using LEGACY ProceduralShelfGenerator only
     */
    private async createLegacyShelfWithGames(position: THREE.Vector3, rowIndex: number, shelfIndex: number, games: any[] = []): Promise<THREE.Group> {
        const shelfGroup = new THREE.Group()
        shelfGroup.name = `legacy-shelf-${rowIndex}-${shelfIndex}`
        shelfGroup.position.copy(position)
        
        try {
            // Use ONLY legacy procedural generation - no instancing
    ...
    /**
     * Create shelf using legacy ProceduralShelfGenerator (main path for legacy renderer)
     */
    ...
    private async createGameBoxesWithNames(
        ...
        for (let i = 0; i < games.length; i++) {
            ...
            const name = `legacy-game-${game.name?.replace(/[^a-zA-Z0-9]/g, '-') || 'unknown'}-${side}-${i}`

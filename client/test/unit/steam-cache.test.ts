...
describe('AppDetailsCache', () => {
    ...
    // Direct access to the private IndexedDbCache instance backing AppDetailsCache, for tests
    // that need to inject a raw record (e.g. a deliberately stale/legacy shape) the public API
    // has no way to produce.
    const rawStore = () => (AppDetailsCache as any).store

    describe('get / set', () => {
        it('should return null for non-existent appid', async () => {
            const result = await AppDetailsCache.get(999)
            expect(result).toBeNull()
        })

        it('should return cached data with isStale=false when schema versions match', async () => {
            await AppDetailsCache.set(10, mockAppDetails)
            const result = await AppDetailsCache.get(10)
        ...
        it('should return data with isStale=true when schema versions mismatch', async () => {
            ...
            await new Promise<void>((resolve) => {
                const tx = rawStore().db.transaction('appdetails', 'readwrite')
    ...
    describe('getMany / setMany', () => {
        it('should return empty map when requested array is empty', async () => {
            const results = await AppDetailsCache.getMany([])
        ...
        it('should cache and return multiple appids correctly', async () => {
            ...
            const results = await AppDetailsCache.getMany([10, 20, 30])
        ...
        it('should correctly flag mixed stale and fresh results', async () => {
            // Inject fresh 10
            await AppDetailsCache.set(10, { ...mockAppDetails, appid: 10 })
            ...
            const results = await AppDetailsCache.getMany([10, 20])

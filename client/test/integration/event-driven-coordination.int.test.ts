...
describe('Event-Driven Coordination Integration (Phase 3f)', () => {
    ...
    // Timeline tracking
    const eventTimeline: Array<{ event: string; batchIndex: number; shelfIndex?: number; timestamp: number }> = []
    ...
    beforeEach(async () => {
        ...
        eventManager.registerEventHandler(
            ...
            (event: CustomEvent<ShelfReadyEvent>) => {
                ...
                eventTimeline.push({
                    event: 'ShelfReady',
                    shelfIndex: event.detail.shelfIndex,
                    batchIndex: event.detail.shelfIndex,
                    timestamp: Date.now()
                })
                console.log(`[EVENT] ShelfReady: shelf ${event.detail.shelfIndex}`)
            }
        )
    ...
    describe('Single Batch Flow', () => {
        ...
        it('should have matching batch indices across all events', async () => {
            ...
            expect(uniqueBatchIndices(batchReadyEvents)).toEqual([0])
            expect(shelfReadyEvents[0].shelfIndex).toBe(0)

...
- **Survey before you extend**: Before adding a new way to do something the codebase already does — a new load path, a new persistence key, a new validation routine, a new event type — find and read every existing implementation of that same concept first. This includes narrower/differently-scoped variants: a new event that means "clear/reset/forget X" when one already exists for a broader or adjacent X is the same failure as a new load path — prefer a discriminating field over a sibling event. Extend the class that already owns it (see "Owner-managed subscriptions" above); a second, differently-shaped mechanism for the same job is a design smell that compounds, not a shortcut. See `.github/lessons-learned.md` for concrete examples.
